...
- **Status transitions** (each a dedicated action, not a generic "update status" field):
  ...
  `SENT → EXPIRED` — a real, persisted transition, applied **lazily and atomically rather
  than by a scheduled job** (no background-job infra exists anywhere in this codebase):
  every path that loads a `SENT` quotation — `getQuotation`, `listQuotations`, and the
  start of `acceptQuotation`/`rejectQuotation`/`salesOrderService.createFromQuotation` —
  first checks whether `validUntil < today` and, if so, persists `status = EXPIRED` on
  that same row before proceeding. This keeps `EXPIRED` a real, queryable stored value
  (so a status filter or report can find expired quotations) rather than a label some
  callers compute and others forget to. An Accept/Convert attempt that lands on a
  quotation which just lazily expired this way is rejected with the same friendly
  "quotation has expired, please re-quote" error a terminal-state quotation always
  produces — no separate error path is needed.
...
- **Line calculation** (server-side only, never trusted from the client): for each line,
  ...
  amount subtracted — documented order, since both are optional and independent).
  **Validated before either engine call**: `discountAmount + (discountPercent% of
  quantity × rate) ≤ quantity × rate` — a combined discount that would exceed the line's
  own gross value is rejected with a friendly, line-specific error; `taxableAmount_pre` is
  never allowed to go negative, it is never silently clamped to zero. Once that check
  passes, `GstEngine.calculateLine({ amount: taxableAmount_pre, isInclusive: false,
  ratePercent, cessPercent, supplyType, isReverseCharge: false })` produces the stored tax
  breakdown.
...
decimals, plus a per-line object-level refine rejecting any combination where
`discountAmount + discountPercent% of quantity×rate` exceeds `quantity × rate` (the
negative-`taxableAmount_pre` guard from Business Rules, enforced here too so a bad
combination never reaches the engine call).
...
- A scheduled/background job to auto-expire quotations (no job infra exists; `EXPIRED` is
  applied lazily and atomically by the read/action paths listed in Business Rules
  instead, never by a cron or worker process)
