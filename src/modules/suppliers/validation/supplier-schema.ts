...
// Same scaled-with-tolerance float check as customer-schema.ts /
// gst-rate-schema.ts (18.15 * 100 === 1814.9999… in binary floats).
...
// (customer-schema.ts's convention), unlike company-schema.ts's as-entered rule.
...
// No supplierType (the RETAIL/WHOLESALE/DEALER/DISTRIBUTOR tiers are a
// selling-side Pricing Engine concept) and no creditLimit (a cap we impose on
// a debtor, not something we'd impose on our own supplier) — the two
// deliberate omissions versus createCustomerSchema, per
// 27-supplier-management.md's Data Model section.
export const createSupplierSchema = z.object({
  displayName: DISPLAY_NAME_SCHEMA,
  // The server re-validates active + "Sundry Creditors or descendant" — this
...
// Create and Update accept the same field set (27-supplier-management.md,
// mirroring customer-schema.ts) — including ledgerGroupId, since re-parenting
// within "Sundry Creditors" is allowed and re-validated server-side. Kept as
// a separate named schema so a future spec can diverge them without touching
// callers.
